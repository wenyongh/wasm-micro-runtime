...
#undef HANDLE_OPCODE
    ...
    || WASM_ENABLE_BULK_MEMORY != 0
            ...
            cell_num_of_local_stack = cur_func->param_cell_num
                                      + cur_func->local_cell_num
                                      + max_stack_cell_num;
            /* Make frame csp pointer size aligned in case
               CPU doesn't support unaligned address access */
            cell_num_of_local_stack = align_uint(
                cell_num_of_local_stack, sizeof(uintptr_t) / sizeof(uint32));
            ...
            frame_csp = frame->csp_bottom =
                (WASMBranchBlock *)(uintptr_t)align_uint64(
                    (uint64)(uintptr_t)frame->sp_boundary, sizeof(uintptr_t));
            frame->csp_boundary =
