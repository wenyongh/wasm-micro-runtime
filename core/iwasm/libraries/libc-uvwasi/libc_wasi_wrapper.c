                    ...
                    uint32 *argc_app, uint32 *argv_buf_size_app)
{
    ...
    err = uvwasi_args_sizes_get(uvwasi,
                                      &argc, &argv_buf_size);
    if (err)
        return err;
                 ...
                 uint32 *environ_offsets, char *environ_buf)
{
    ...
    err = uvwasi_environ_sizes_get(uvwasi,
                         ...
                         wasi_fd_t fd, char *path, uint32 path_len)
{
    ...
    return uvwasi_fd_prestat_dir_name(uvwasi,
                                            fd, path, path_len);
}
                           ...
                           wasi_fd_t fd, const char *path, uint32 path_len)
{
    ...
    return uvwasi_path_create_directory(uvwasi, fd,
                                              path, path_len);
}
